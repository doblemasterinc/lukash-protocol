...
2. [x] **Sprint 2 (07-a) Capa 1**: Switch B0→B2 por valoración de mercado + token accounting + doble candado 7d. Implementado + desplegado en devnet.
   ...
   - [~] **Testing v9 en devnet**: updateOracleState ✅. refreshVaultValuation ❌ (Pyth devnet feed status != Trading). **Fix pendiente: comentar `require!(status == PYTH_STATUS_TRADING, ...)` en `parse_pyth_price`, rebuild, redeploy, retest.**
