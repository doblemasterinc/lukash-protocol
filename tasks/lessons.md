...
## 2026-08-26 (sesión 13) — Solana Playground sandbox bloquea `anchor.utils.token`
El sandbox de Playground (beta.solpg.io) rechaza `anchor.utils.token.TOKEN_PROGRAM_ID` con el error críptico "'location' is not allowed" (fallo en el procesador, antes de ejecutar). Fix: usar `new anchor.web3.PublicKey("TokenkegQfeZyiNwAJbNbGKPFXCWuBvf9Ss623VQ5DA")` directamente. Lección: en Playground, usar siempre PublicKeys explícitas para programas del sistema (Token Program, System Program, etc.) — no confiar en utilidades de `anchor.utils` que pueden acceder a APIs del navegador bloqueadas por el sandbox.

## 2026-08-26 (sesión 14) — Pyth devnet y mainnet usan program IDs distintos
El Pyth V2 Oracle Program tiene program IDs diferentes: devnet `gSbePebfvPy7tRqimPoVecS2UsBvYv46ynrzWocc92s` vs mainnet `FsJ3A3u2vn5cTVofAjvy6y5kwABJAqYWpe4975bi2epH`. El `#[account(owner = ...)]` constraint necesita un `declare_id!` con el ID correcto para cada entorno. Patrón elegido: módulo `pyth_oracle` con `declare_id!` que se cambia al migrar a mainnet. Lección: al validar owners de cuentas de programas externos, siempre verificar si el program ID difiere entre devnet y mainnet antes de hardcodear.
...
## 2026-08-31 (sesión 22) — Usar imagen dedicada en vez de recortar CSS de una imagen compuesta
Intenté extraer la palabra "LUKASH" del logotipo completo usando `object-fit:cover` + `object-position` CSS para cropear solo el texto. Tras 3 intentos ajustando porcentajes (68%, 78%), ninguno daba el resultado limpio. La solución correcta: usar una imagen dedicada que ya contenga exactamente lo que se necesita (`LUKASH palabra.png`). Lección: si necesitas mostrar solo una parte de una imagen compuesta, buscar o crear un asset dedicado es más rápido y predecible que ajustar crops CSS a ciegas — especialmente cuando no se conoce la distribución exacta del contenido en la imagen.

## 2026-09-08 (sesión 28) — Nginx exact match + alias = path doubling
`location = / { alias /opt/lukash/splash/index.html; }` genera `index.htmlindex.html` porque nginx concatena el URI (/) al alias. `try_files` con root también falla por la misma razón. **Solución: `root /opt/lukash/splash; rewrite ^ /index.html break;`** — el rewrite reemplaza el URI antes de buscar el archivo, evitando la concatenación. Lección: para servir un archivo específico en un location exacto, usar `root + rewrite break`, nunca `alias` a un archivo.

## 2026-09-08 (sesión 28) — Basic auth en data rooms para inversores genera fricción innecesaria
El popup de credenciales de HTTP basic auth confunde a inversores no técnicos, es incompatible con herramientas de testing automatizado (Browser pane muestra 401 directo), y no aporta seguridad real si la URL ya es secreta. **URL obscurity (`/ruge2026/`) es suficiente para un data room pre-seed.** Si se necesita auth real en el futuro, usar un login page propio con tokens, no HTTP basic auth.

## 2026-09-08 (sesión 30) — Segregar contenido público vs privado en outreach desde el primer draft
Al preparar tweets inaugurales y mensajes DM para ángeles, se incluyeron links a GitHub y data room en borradores de tweets públicos. Sebastián lo detectó: "no quiero que se publique el git ni la página en X, es algo privado para los inversionistas". Lección: al preparar material de outreach, separar DESDE EL INICIO lo público (tweets, replies, bio) de lo privado (GitHub, data room, pitch deck, métricas). Links privados = solo DM con inversores verificados. En público: "DM for access", nunca el link directo.

## 2026-09-08 (sesión 30) — No sobre-prometer features en roadmap de outreach
LUKAI v1.0 es un orquestador de estados on-chain (keeper), NO una IA conversacional. La interfaz IA es v2.0 (Etapa 2A). Describir LUKAI como "AI assistant" en mensajes a inversores compromete a entregar algo que no existe aún. Sebastián: "LUKAI no es aún una IA, no quiero comprometer de más". Regla: en material de outreach, describir solo lo que ESTÁ construido o en construcción inmediata. Features futuras → "roadmap" o "planned", nunca como feature actual.

## 2026-09-11 (sesión 31) — nginx `proxy_pass` con upstream externo resuelve DNS solo al arranque
Si `proxy_pass` apunta a un dominio externo (ej. `https://api.mainnet-beta.solana.com/`) sin `resolver`, nginx intenta resolver el DNS **una vez al arranque**. Si la resolución falla (DNS intermitente, timeout), nginx se niega a iniciar y se cae completamente — afectando TODAS las rutas, no solo la del proxy. **Fix: usar `resolver 8.8.8.8 1.1.1.1 valid=300s` + `set $variable` para forzar resolución en runtime.** La variable es clave: sin ella, nginx sigue resolviendo al arranque aunque haya resolver.

## 2026-09-11 (sesión 31) — Consultar documentación del proyecto ANTES de cambiar el VPS
Al recibir "eliminar fricción", cambié nginx sin consultar `tasks/todo.md` ni `key-decisions.md`. La documentación mostraba que la splash era parte del diseño completado y probado (`[x] Splash page en / como entry point`), y que la "fricción eliminada" era el basic auth, no el splash. Resultado: revertir el cambio. **Regla: ante cualquier cambio en VPS/producción, verificar el diseño documentado primero.** Aplica especialmente cuando se trabaja en un proyecto con decisiones ya cerradas (ADRs).

## 2026-09-11 (sesión 31) — Usar isotipo (cuadrado) para círculos, logotipo (horizontal) para banners
El splash usaba `lukash-logotipo.png` (1408×768, felino + texto horizontal) en un `<div>` circular de 200px con `object-fit:cover` → se recortaba mostrando solo una porción. El archivo correcto era `ISOTIPO V2.png` (cuadrado, felino-bóveda-infinito). **Regla: isotipo = formato cuadrado (para avatares, círculos, favicons). Logotipo = formato horizontal (para headers, banners).** Verificar las dimensiones del asset antes de usarlo en un contenedor con aspect ratio fijo.
